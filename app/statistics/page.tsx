...
interface Stats {
  ...
  week: { count: number }
  month: { count: number }
  topCustomersByCount: { customer_name: string; order_count: number }[]
...
function formatLabel(label: string, period: string): string {
  if (period === 'daily') return label  // already "HH:00"
...
function CustomTooltip({ active, payload, label, period }: { active?: boolean; payload?: { value: number }[]; label?: string; period: string }) {
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #e8edf5', borderRadius: 12, padding: '10px 16px', boxShadow: '0 4px 20px rgba(0,0,0,0.12)', fontSize: 13 }}>
      <p style={{ fontWeight: 700, color: '#0f1f3d', marginBottom: 6 }}>{formatLabel(label ?? '', period)}</p>
      <p style={{ color: '#10b981', fontWeight: 600 }}>הזמנות: {payload[0].value}</p>
    </div>
  )
}
  ...
  return (
    <div className="page-container">
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p className="page-subtitle" style={{ marginRight: 16 }}>ניתוח פעילות העסק</p>
      </div>

      {/* KPIs */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'הזמנות החודש',   value: stats.month.count,               sub: '30 הימים האחרונים', color: '#2d6aff' },
          { label: 'הזמנות השבוע',   value: stats.week.count,                sub: '7 הימים האחרונים',  color: '#10b981' },
      ...
      <div className="card" style={{ padding: 28, marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 16, color: '#0f1f3d' }}>
                {period === 'daily' ? 'הזמנות לפי שעה — היום' : period === 'weekly' ? 'הזמנות לפי יום — שבוע אחרון' : 'הזמנות לפי יום — החודש הנוכחי'}
              </div>
              <div style={{ fontSize: 12, color: '#9baabe' }}>ציר X: {period === 'daily' ? 'שעות' : 'תאריכים'} · ציר Y: הזמנות</div>
            </div>
          </div>
        ...
        {stats.trend.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={stats.trend} margin={{ top: 5, right: 10, left: -10, bottom: 5 }} barCategoryGap="30%">
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f4fb" vertical={false} />
              <XAxis
                dataKey="label"
                tick={{ fontSize: 10, fill: '#9baabe' }}
                axisLine={false}
                tickLine={false}
                ...
                height={period === 'monthly' ? 40 : 20}
              />
              <YAxis tick={{ fontSize: 11, fill: '#9baabe' }} axisLine={false} tickLine={false} allowDecimals={false} />
              <Tooltip content={<CustomTooltip period={period} />} cursor={{ fill: '#f0f4ff' }} />
              <Bar dataKey="orders" name="הזמנות" radius={[4, 4, 0, 0]}>
                {stats.trend.map((_, i) => (
                  <Cell key={i} fill={`url(#trendGrad)`} />
                ))}
              </Bar>
              <defs>
          ...
          {stats.pendingCount + stats.deliveredCount === 0 ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <ResponsiveContainer width="55%" height={160}>
                <PieChart>
                  <Pie data={statusPie} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="value" paddingAngle={5} startAngle={90} endAngle={-270}>
